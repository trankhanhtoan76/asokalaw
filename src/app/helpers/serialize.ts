export const serialize = {
    ...
    stringify(val) {
        ...
        function serialize(val, canReference = 1) {
            ...
            return (a ? "a" : `O:${serializeString(val.constructor.name)}`)
                + `:${vals.length}:{${vals.map(([k, v]) => serialize(a && /^\d{1,16}$/.test(k) ? +k : k, 0) + serialize(v)).join("")}}`;
        }
    ...
    parse(str, allowedClasses = {
        ...
        allowedClasses.stdClass = serialize.stdClass; // Always allowed.
        let offset = 0;
        const values = [null];
                ...
                : typ === "s" ? readString(";")
                    : typ === "b" ? readBoolean()
                        : typ === "i" ? readInt()
                            : typ === "d" ? readDecimal()
                                : typ === "a" ? readArray()
                                    : typ === "o" ? readObject(create(readString()))
                                        : typ === "c" ? readCustomObject(create(readString()))
                                            : typ === "r" ? values[readInt()]
